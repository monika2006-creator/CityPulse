...
export default function LiveSignalsPanel({ signals, total, selectedId, onSelect, className = "" }) {
  return (
    <section
      ...
      className={`rounded-xl border border-border bg-surface shadow-float ${className}`}
    >
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
        <h3 className="flex items-center gap-2 text-[12px] font-bold tracking-[0.12em] text-ink">
          <span className="h-1.5 w-1.5 rounded-full bg-cyan shadow-dot" />
          LIVE SIGNALS
        </h3>
        <span className="font-mono text-[10.5px] text-muted">
          <span className="text-cyan-ink">{signals.length}</span> of {total}
        </span>
      </div>
