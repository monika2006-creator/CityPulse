...
const NAV_ITEMS = [
  ...
  { id: "situations", label: "Situations", icon: AlertTriangle, badge: "04" },
  ...
  return (
    <button
      ...
      className={`group relative flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
        isActive
          ? "bg-cyan/10 text-ink shadow-[0_0_0_1px_rgba(34,211,238,0.15),0_0_16px_-4px_rgba(34,211,238,0.35)]"
          : "text-muted hover:translate-x-0.5 hover:bg-white/[0.04] hover:text-ink"
      }`}
    >
      {isActive && (
        <span
        ...
        <span
          className={`rounded-md px-1.5 py-0.5 font-mono text-[10px] font-semibold leading-none transition-colors duration-200 ${
            isActive
              ...
              : "bg-white/5 text-muted group-hover:text-ink"
          }`}
        >
          {item.badge}
        </span>
        ...
        <div className="flex items-center justify-between px-5 pb-5 pt-6">
          <div className="flex items-center gap-2.5">
            <span className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-elevated">
              <Radio size={17} className="text-cyan" strokeWidth={2.25} />
              <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-cyan shadow-[0_0_6px_1px_rgba(34,211,238,0.7)]" />
            </span>
        ...
        <div className="border-t border-border px-3 py-4">
          <div className="mb-3 rounded-lg border border-border bg-surface px-3 py-2.5">
            ...
            <div className="mt-1.5 flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-ink">Systems Operational</span>
            </div>
            <div className="mt-2.5 flex items-center gap-1.5 border-t border-border pt-2.5">
              <MapPin size={13} className="text-violet shrink-0" />
              <div className="leading-tight">
                <p className="text-xs font-medium text-ink">Jaipur</p>
                <p className="text-[10.5px] text-muted">Live monitoring</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            <button
              type="button"
              className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-muted transition-all duration-200 hover:bg-white/[0.04] hover:text-ink"
            >
              <Bell size={16} />
